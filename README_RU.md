...
## 14 Grand Challenges: Наука и индустрия (Out-of-Sample бенчмарк)

Масштабный эмпирический бенчмарк на 14 трансцендентных обратных задачах физики, медицины и прикладной инженерии ($50,000$ отложенных тестовых точек на каждую задачу, Apple Silicon M1):

| # | Задача (Grand Challenge) | Прикладная область | Базовая MAE | KAN MAE | Символическая MAE | Метрика $R^2$ | Фактор снижения ошибки |
| :---: | :--- | :--- | :---: | :---: | :---: | :---: | :---: |
| 1 | **Mualem-van Genuchten** | Гидрогеология ($K_r \to h$) | 0.00455 | **0.00000** | **0.00005** | **100.0000%** | **99.1x (Симв) / >10,000x (KAN)** |
| 2 | **Inverse Fermi-Dirac** | Квантовые полупроводники ($n \to \eta$) | 0.00697 | **0.00014** | **0.00278** | **99.9998%** | **2.5x (Симв) / 48.9x (KAN)** |
| 3 | **Hyperbolic Kepler** | Астродинамика NASA ($M \to F$) | 0.00069 | **0.00009** | **0.00063** | **99.9999%** | **1.1x (Симв) / 7.3x (KAN)** |
| 4 | **Peng-Robinson EOS** | Сверхкритическая гидродинамика ($P \to v$) | 0.00040 | **0.00036** | 0.00159 | **99.9996%** | **1.11x (KAN)** |
| 5 | **Voigt Spectral Profile** | Спектроскопия HITRAN ($x \to V$) | 0.00033 | **0.00010** | 0.00464 | **97.45%** | **3.22x (KAN)** |
| 6 | **Michaelis-Menten PK** | Фармакокинетика ($t \to C(t)$) | 0.00051 | **0.00001** | 0.00053 | **100.0000%** | **34.6x (KAN)** |
| 7 | **Radiation LKB NTCP** | Лучевая онкология ($D \to \text{NTCP}$) | 0.00079 | 0.00182 | **0.00068** | **99.9985%** | **1.17x (Симв)** |
| 8 | **Cardiac MRI $T_1$ Map** | Томография сердца ($S \to T_1$) | 0.50290 | **0.00564** | **0.33187** | **100.0000%** | **1.52x (Симв) / 89.2x (KAN)** |
| 9 | **Coronary Artery FFR** | Кардиология ($S \to \text{FFR}$) | 0.00004 | **0.00000** | **0.00002** | **100.0000%** | **1.88x (Симв) / 934x (KAN)** |
| 10 | **Solar PV Diode** | Солнечные батареи SPICE ($V \to I$) | 0.00307 | **0.00222** | 0.01047 | **99.9931%** | **1.38x (KAN)** |
| 11 | **Li-Ion Battery SoC** | Электромобили BMS ($V_{oc} \to \text{SoC}$) | 0.00196 | **0.00022** | **0.00044** | **99.9992%** | **4.45x (Симв) / 9.0x (KAN)** |
| 12 | **Black-Scholes IV** | Количественные финансы ($C/S \to \sigma$) | 0.00005 | **0.00000** | **0.00000** | **100.0000%** | **1920x (Симв) / 2856x (KAN)** |
| 13 | **Prandtl-Meyer M(nu)** | Гиперзвук / CFD ($\nu \to M$) | 0.00025 | **0.00004** | 0.00039 | **100.0000%** | **6.62x (KAN)** |
| 14 | **Oxygen Dissociation** | ИВЛ / ЭКМО / Реанимация ($sO_2 \to pO_2$) | 0.14570 | **0.00002** | **0.00278** | **100.0000%** | **52.4x (Симв) / 7731x (KAN)** |
