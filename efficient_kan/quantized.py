...
Provides ~3.5-4x memory compression over FP32 and ~1.9x over FP16 while maintaining
near-lossless accuracy on inference.
"""
...
class QuantizedWeight(nn.Module):
    ...
    Packs floating-point weights into 32-bit words (`uint32`) containing 4 INT8 values
    (for `bits=8`) or 8 INT4 values (for `bits=4`), with per-group scales and biases.
    ...
    Args:
        ...
        group_size: Number of weight elements per quantization group (default: 64, or 32).
        bits: Bit-width per parameter (default: 8 for INT8, or 4 for INT4).
...
class QuantizedKANLinear(nn.Module):
    ...
    def from_layer(
        ...
        quant_base = QuantizedWeight(layer.base_weight, group_size=group_size, bits=bits, mode=mode)
        spline_w = layer.scaled_spline_weight.reshape(layer.out_features, -1)
        quant_spline = QuantizedWeight(spline_w, group_size=group_size, bits=bits, mode=mode)
        return cls(
...
class QuantizedFastKANLinear(nn.Module):
    ...
    def from_layer(
        ...
        quant_base = QuantizedWeight(layer.base_weight, group_size=group_size, bits=bits, mode=mode)
        quant_spline = QuantizedWeight(layer.spline_weight, group_size=group_size, bits=bits, mode=mode)
        return cls(
            in_features=layer.in_features,
            out_features=layer.out_features,
...
class QuantizedChebyKANLinear(nn.Module):
    ...
    def from_layer(
        ...
        quant_base = QuantizedWeight(layer.base_weight, group_size=group_size, bits=bits, mode=mode)
        quant_cheby = QuantizedWeight(layer.cheby_weight, group_size=group_size, bits=bits, mode=mode)
        return cls(
            in_features=layer.in_features,
            out_features=layer.out_features,
            degree=layer.degree,
...
class QuantizedWavKANLinear(nn.Module):
    ...
    def from_layer(
        ...
        quant_base = QuantizedWeight(layer.base_weight, group_size=group_size, bits=bits, mode=mode)
        quant_wav = QuantizedWeight(layer.wav_weight, group_size=group_size, bits=bits, mode=mode)
        return cls(
            in_features=layer.in_features,
            out_features=layer.out_features,
...
class QuantizedFourierKANLinear(nn.Module):
    ...
    def from_layer(
        ...
        quant_base = QuantizedWeight(layer.base_weight, group_size=group_size, bits=bits, mode=mode)
        quant_fourier = QuantizedWeight(layer.fourier_weight, group_size=group_size, bits=bits, mode=mode)
        return cls(
            in_features=layer.in_features,
            out_features=layer.out_features,
            freqs=layer.freqs,
...
class QuantizedJacobiKANLinear(nn.Module):
    ...
    def from_layer(
        ...
        quant_base = QuantizedWeight(layer.base_weight, group_size=group_size, bits=bits, mode=mode)
        quant_jacobi = QuantizedWeight(layer.jacobi_weight, group_size=group_size, bits=bits, mode=mode)
        return cls(
            in_features=layer.in_features,
            out_features=layer.out_features,
            degree=layer.degree,
...
class QuantizedLowRankKANLinear(nn.Module):
    ...
    def from_layer(
        ...
        quant_base = QuantizedWeight(layer.base_weight, group_size=group_size, bits=bits, mode=mode)
        quant_V = QuantizedWeight(layer.spline_V, group_size=group_size, bits=bits, mode=mode)
        quant_U = QuantizedWeight(layer.spline_U, group_size=group_size, bits=bits, mode=mode)
        return cls(
            in_features=layer.in_features,
            out_features=layer.out_features,
            rank=layer.rank,
...
class QuantizedMultKANLinear(nn.Module):
    ...
    def from_layer(
        ...
        quant_sub = QuantizedFastKANLinear.from_layer(
            layer.sub_layer, group_size=group_size, bits=bits, mode=mode
        )
        return cls(
            in_features=layer.in_features,
            out_features=layer.out_features,
...
def quantize(
    ...
    **kwargs,
    ...
    Args:
        ...
        bits: Bit-width per parameter (8 for INT8, 4 for INT4; default: 8).
        mode: Quantization mode ("affine" or "symmetric", default: "affine").

    Returns:
        The quantized model (updated in-place).
    """
    nn.quantize(model, group_size=group_size, bits=bits, mode=mode, **kwargs)
    return model
