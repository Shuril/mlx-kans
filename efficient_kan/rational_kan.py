...
class RationalKANLinear(nn.Module):
    ...
    def __call__(self, x: mx.array) -> mx.array:
        ...
        T = compute_cheby_basis(x_flat, self.max_degree)

        # Numerator: P(x) = sum_{k=0}^p a_k T_k(x)
        T_num = T[:, :, : self.p_degree + 1]
        P = mx.einsum("bik,oik->boi", T_num, self.num_weight)

        # Denominator: Q(x) = sum_{m=1}^q b_m T_m(x)
        T_den = T[:, :, 1 : self.q_degree + 1]
        Q = mx.einsum("bim,oim->boi", T_den, self.den_weight)

        # Rational activation: phi(x) = P(x) / (1 + |Q(x)|)
        phi = P / (1.0 + mx.abs(Q))
        rational_out = mx.sum(phi, axis=-1)
