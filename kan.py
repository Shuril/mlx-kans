...
class KANLinear(nn.Module):
    ...
    def b_splines(self, x: mx.array) -> mx.array:
        ...
        Returns:
            B-spline bases of shape (..., in_features, grid_size + spline_order)
        """
        if self.use_metal_kernel and self.spline_order == 3 and x.ndim == 2:
            from efficient_kan.metal_kernels import metal_bspline_basis, is_metal_available
            if is_metal_available():
                return metal_bspline_basis(x, self.grid_range[0], self.grid_range[1], self.grid_size)
